// relief/export.js — writes slicer-ready files, adapted from terra/export.js.
//
...
const ATTRIBUTION = 'Generated with Relief - joshhicks.info/relief';
const round = v => Math.round(v * 10000) / 10000;

/* ── Binary STL ────────────────────────────────────────────────────────── */

export function exportStl(positions, { name = 'relief' } = {}) {
  const triangles = positions.length / 9;
  const buf = new ArrayBuffer(84 + triangles * 50);
  const view = new DataView(buf);
  const header = `Relief ${name} - joshhicks.info`;
  for (let i = 0; i < 80; i++) {
...
export function export3mf(positions, { name = 'relief', color = '#E08A2E', stats } = {}) {
  const notes = stats
...
<metadata name="Application">Relief - joshhicks.info/relief</metadata>
